...
case class Subject(id: Int, x: Int, y: Int){
  ...
  def behaviorPage() = Callback {
    println("id: " + sid)
    val url = "http://localhost:12345/#subjects/" + sid
...
object ProcessInstance {
  ...
  val subjectMap : Map[Int, ListBuffer[State]] = Map()
  val subOutgoingMap : Map[Int, Map[Int, Int]] = Map()
  val subIncomingMap : Map[Int, Map[Int, Int]] = Map()
  val bottomArrowMap : Map[Int, (Int, Int)] = Map()
  val topArrowMap : Map[Int, (Int, Int)] = Map()
  val arrowList : ListBuffer[VdomElement] = ListBuffer()
...
//    )
  ...
  def addConnect(sid: Int): Unit ={
    ...
    for(state <- subjectMap(sid)){
      state.sTransition.filterNot(t => t.stosMsg.subid == sid).foreach(
        t =>{
...
//    }
  ...
  def connectAll(): Unit ={
    ...
    for (sub <- subjectsMirror){
      dom.console.info("connect subid."  + sub.sid)
      if(subOGMapMirror.contains(lastSub.sid)){
        val bottomRight = subOGMapMirror(lastSub.sid).filterKeys(k => k > lastSub.sid).values.sum
        val topLeft = subOGMapMirror(lastSub.sid).filterKeys(k => k < lastSub.sid).values.sum
        var bottomCount = 0
        var topCount = 0
        dom.console.info("connect id " + bottomRight )
        for(target <- subOGMapMirror(lastSub.sid).keys.toSeq.sorted){

          val tarSub = subjects.find(s => s.sid == target).get
          for(i <- 1 to subOGMapMirror(lastSub.sid)(target)){
            if(target == sub.sid){
              dom.console.info("arrow left " + lastSub.yy + "   " + tarSub.yy)
              arrowList += Arrow(lastSub.xx + lastSub.width, (lastSub.yy + lastSub.height/2/(subOGMapMirror(lastSub.sid)(target)+1) * i),
                sub.xx, (sub.yy + sub.height/2/(subOGMapMirror(lastSub.sid)(target)+1) * i), 0).showArrow()
            }else if(target > lastSub.sid){
      ...
      if(subOGMapMirror.contains(sub.sid)){
        if(subOGMapMirror(sub.sid).contains(lastSub.sid)){
          for(i <- 1 to subOGMapMirror(sub.sid)(lastSub.sid)){
            arrowList += Arrow(sub.xx, (sub.yy + lastSub.height/2 + lastSub.height/2/(subOGMapMirror(sub.sid)(lastSub.sid)+1)*i),
              lastSub.xx + lastSub.width, (lastSub.yy + lastSub.height/2 + lastSub.height/2/(subOGMapMirror(sub.sid)(lastSub.sid)+1)*i), 0).showArrow()
          }
          subOGMapMirror(sub.sid).remove(lastSub.sid)
          dom.console.info("removed OGMap " + subOGMapMirror)
        }
      }
      if(sub.equals(subjectsMirror.last)){
        if(subOGMapMirror.contains(sub.sid)){
          val topLeft = subOGMapMirror(sub.sid).values.sum
          var topCount = 0
          for(target <- subOGMapMirror(sub.sid).keys.toSeq.sorted){
            val tarSub = subjects.find(s => s.sid == target).get
            for(i <- 1 to subOGMapMirror(sub.sid)(target)){
              topArrowMap += target -> (topArrowMap(target)._1, topArrowMap(target)._2 + 1)
              topCount += 1
  ...
  def addOutgoing(id: Int): Map[Int, Int] ={
    ...
    if(subjectMap.contains(id)) {
      ...
      for(s <- subjects){
        s.sTransition.foreach(t => if(transOutgoingMap.contains(t.tid)) transOutgoingMap(t.tid) += 1 else transOutgoingMap += (t.tid -> 0))
      }
    }
    transOutgoingMap
  }
